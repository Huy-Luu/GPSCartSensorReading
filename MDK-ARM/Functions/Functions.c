...
float convert(int16_t *x_axis, int16_t *y_axis, int16_t *z_axis, uint8_t compassData[6])
{
	...
	*z_axis = ((uint16_t)compassData[5]<<8)|compassData[4];
	...
	//*x_axis = x_s*((float)*x_axis - x_h);
	//*y_axis = y_s*((float)*y_axis - y_h);
	
	heading = atan2f((float)*y_axis, (float)*x_axis) *180/3.14;
//	if(heading<0.0)
//	{
//		//heading+=360.0;
//		heading = 360 - heading;
...
void nrfReadRegMulti(uint8_t reg, uint8_t *data, uint8_t size)
{
	csSelect();
...
void nrfInit(void)
{
	...
	nrfWriteReg(SETUP_AW, 0x03); //5 bytes for tx/rx address
	
	nrfWriteReg(SETUP_RETR, 0); //no retransmission
	
	nrfWriteReg(RF_CH, 0); //will be setup during Tx or Rx
	
	nrfWriteReg(RF_SETUP, 0x0E); //Power = 0db, data rate = 2Mbps
	
	//enable device after config
	ceEnable();
	csSelect();
...
//setup Tx mode
void nrfTxMode(uint8_t *address, uint16_t channel)
{
	//disable then configure
	ceDisable();
	
	nrfWriteReg(RF_CH, channel); //select channel
	nrfWriteRegMulti(TX_ADDR, address, 5); //write the tx channel
	
	//power up device
	uint8_t config = nrfReadReg(CONFIG);
	config = config | (1<<1);
	nrfWriteReg(CONFIG, config);
	
	//enable after configure
	ceEnable();
}

//transmit data
uint8_t nrfTransmit(uint8_t *data)
{
	uint8_t to_send = 0;
  ...
  //select device
	...
	//payload command
	to_send  = W_TX_PAYLOAD;
	HAL_SPI_Transmit(&hspi1, &to_send, 1, 100);
	...
	uint8_t fifo_status = nrfReadReg(FIFO_STATUS);
...
void nrfRxMode(uint8_t *address, uint8_t channel)
{
	...
	//select data pipe 1
	uint8_t en_rxaddr = nrfReadReg(EN_RXADDR);
	en_rxaddr = en_rxaddr| (1<<1);
	nrfWriteReg(EN_RXADDR, en_rxaddr);
	
	nrfWriteRegMulti(RX_ADDR_P1, address, 5); //write the tx address
	
	nrfWriteReg(RX_PW_P1, 32); //32 bit payload size for pipe 1
